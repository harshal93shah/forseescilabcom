function [y] = AlgebraicInterleaver(in,method,len,varargin)
    y=[];
    // Display mode
    mode(0);
    ...
    //AlgebraicInterleaver The AlgebraicInterleaver object rearranges the elements
    // of its input vector using an algebraically derived permutation.The algebraic 
    //interleaver performs all computations in modulo N, where N is the length
    //in:input -  The input must be a  vector in the range [0, N–1].

    //Y = AlgebraicInterleaver(in,'taco',len,MF,CS) outputs the elements of input using 
    ...
    ///Y = AlgebraicInterleaver(in,'weco',len,PE) outputs the elements of input using 
    //Welch-Costas method
    //len: Length-Number of elements in input vector.
    //Specify the number of elements in the input as a positive, integer, scalar.
    ...
    if(~isreal(in) | or( isnan(in)) | min(size(in))~=1 | length(in)~= len) then
        error("AlgebraicInterleaver:improper input");
    end
    ...
    if(~strcmp(method,'taco')) then
        ...
        if (RHS ~=5) then
            error("AlgebraicInterleaver:Invalid no. of arguments");
        end
    ...
    elseif(~strcmp(method,'weco')) then
        ...
        for i = 2:(len-1)
            if(modulo(len+1,i)==0) then
                error("AlgebraicInterleaver:length + 1 should be prime");
            end
        end

        //check conditions on PrimitiveElement
        if(~isreal(PE) | or( isnan(PE)) | min(size(PE))~=1 | (PE <= 0) | (PE >= len )) then
            error("AlgebraicInterleaver:improper PrimitiveElement");
        end
        ...
        if(or(num)) then
            error("Descrambler:AlgebraicInterleaver PrimitiveElement");
        end
        ...
        error("AlgebraicInterleaver:improper Method");
    end
